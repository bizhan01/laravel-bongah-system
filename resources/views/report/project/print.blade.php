			...
			<div class="">
							...
							<div class="card-block">
						      ...
						      <tbody>
						        @foreach($projects as $row => $project)
						          <tr>
												<td>{{ $row+1 }}</td>
						            <td><span> </span>{{ $project->date}}</td>
						            <td><span> </span>{{ $project->user_name}}</td>
						            <td><span> </span>{{ $project->prefix}} {{ $project->name}}</td>
						            <td><span> </span>{{ $project->project_type}}</td>
						            <td><span> </span>{{ $project->location}}</td>
						            <td><span> </span>{{ $project->space}}</td>
						            <td><span> </span>{{ $project->start_date}}</td>
						            <td><span> </span>{{ $project->end_date}}</td>
						          </tr>
						        @endforeach
						      </tbody>
								</table>
                ...
                <a href="javascript:window.print()" class="btn btn-inverse waves-effect waves-light">
  							<button type="button" class="btn btn-info label-left float-xs-right mr-0-5">
									...
									<a href="{{ route('projectReport') }}"><button type="button" class="btn-warning" name="button" >برگشت</button></a>
							</div>
						</div>
					</div>
				</div>
